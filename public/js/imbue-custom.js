...
$(function() {
  ...
  // KBW-Countdown Start
  // --------------------------------------------- //
  $('#countdown').countdown({until: $.countdown.UTCDate(+1, 2021, 10, 7), format: 'D'});
  $('#countdown-large').countdown({until: $.countdown.UTCDate(+1, 2021, 10, 7), format: 'DHMS'});
  // --------------------------------------------- //
  // KBW-Countdown End
  // --------------------------------------------- //
  ...
  $(document).ready(function() {
    $('.notify-form').on('submit', function(e){
      ...
      if (!fname || fname == "") {
        $("#errorInput").text("Enter Your Name");
        return;
      }

      if (!email || email == "") {
        $("#errorInput").text("Enter Email");
        return;
      }
      const regex = /^\S+@\S+\.\S+$/;
      if (!regex.test(email)) {
        $("#errorInput").text("Enter Valid Email");
        return;
      }
